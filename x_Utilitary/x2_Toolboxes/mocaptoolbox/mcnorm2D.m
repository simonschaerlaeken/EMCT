function d2 = mcnorm2D(d, comps)
% Calculates the norms of kinematic vectors in 2D.
%
% syntax
% n = mcnorm(d);
% n = mcnorm(d, comps);
...
if nargin==2
    if ~isnumeric(comps) || length(comps)>2 || max(comps)>2 || min(comps)<1
