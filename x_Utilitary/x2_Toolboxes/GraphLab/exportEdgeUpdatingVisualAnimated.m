...
% Exports a GIF of an animated, edge-updating visual. Nodes are layed out
% based on the architecture of the overall graph (the graph which is an
% amalgam of all graph frames) and then kept in the same location
% throughout the entirety of the GIF. Edges are activated and deactivated
% over time. Active edges are red, and old edges are grey. If edges decay
% over time, the grey becomes increasingly transparent.
%
% INPUT:
...
% GRAPH REQUIREMENTS:
%   - None
function exportEdgeUpdatingVisualAnimated(filename,overallGraph,...
		graphFrames,titles,framesPerGraph,decayDur,layout,loopCount)
	if(nargin < 8)
		...
		if(nargin < 7)
			...
			layout = 'auto';
			if(nargin < 6)
				 % Integer number of graph frames needed to fully decay an old edge
	...
	% Draw nodes and render the current title
...
function drawNodes(nodeCoords)
	if(size(nodeCoords,1) < 3)
		% 2D scatter plot the nodes as blue circles
		scatter(nodeCoords(1,:),nodeCoords(2,:),'b');
	else
		% 3D scatter plot the nodes as blue circles
		scatter3(nodeCoords(1,:),nodeCoords(2,:),nodeCoords(3,:),'b');
	end
...
function nodePairs = drawEdges(nodePairs,nodeCoords,decayDur)
	...
	if(size(nodeCoords,1) < 3)
		...
		for curPair = nodePairs
			...
			elseif(curPair(1) == 0)
				% Active edges
				% Draw the edge as a solid red line
				plot(nodeCoords(1,curPair(2:3)),nodeCoords(2,curPair(2:3)),'r-');
			elseif(curPair(1) == 1)
				% Anticipated edges
		...
		for curPair = nodePairs
			...
			elseif(curPair(1) == 0)
				% Active edges
				% Draw the edge as a solid red line
				plot3(nodeCoords(1,curPair(2:3)),nodeCoords(2,curPair(2:3)),nodeCoords(3,curPair(2:3)),'r-');
			elseif(curPair(1) == 1)
				% Anticipated edges
				%Removed due to a cluttered appearance
			end
